...
app.post('/users', async (req,res)=>{
  ...
  try{
    ...
    res.status(201).json({ message: 'created a new user', userId: result.insertedId })
  } catch(e){
...
app.post('/tasks', async (req, res) => {
  ...
  try {
    const result = await tasksCollection.insertOne({ description, completed, userId });
    if (!result) {
      res.status(400);
    }
    res.status(201).json({ message: "created a new task", taskId: result.insertedId });
  } catch (e) {
...
app.delete('/users/:id', async(req,res)=>{
  try{
    const result = usersCollection.deleteOne({ _id: new ObjectId(req.params.id) })
    if (result.deletedCount === 0) {
