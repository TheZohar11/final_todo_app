...
- **Early `return` after every error** - otherwise the code keeps running and acts like the call succeeded (navigating away, saving an undefined token...).
...
- **Save the token** (`localStorage.setItem("authToken", data.token)`) and send it later on protected routes as a header: `Authorization: Bearer <token>`. localStorage survives page refreshes, which is what keeps the user logged in.
